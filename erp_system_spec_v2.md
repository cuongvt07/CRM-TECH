...
1. Sales tạo đơn hàng
        │
        ▼
2. Hệ thống kiểm tra available_quantity
        │
...
## 12. UI/UX DESIGN SPEC

### 12.1 Layout tổng thể
...
| Token | Hex | Dùng cho |
|---|---|---|
| Primary | `#1677FF` | Button, link, active state |
| Success | `#52C41A` | Trạng thái OK, đủ hàng |
| Warning | `#FAAD14` | Cảnh báo, pending |
| Error | `#FF4D4F` | Lỗi, thiếu hàng |
| Neutral | `#8C8C8C` | Text phụ |
...
**Component library:** Ant Design Vue hoặc Naive UI

### 12.3 Sidebar

- Icon + Text luôn hiển thị
- Active state: nền xanh nhạt, text/icon xanh đậm
- Có thể collapse: chỉ icon + tooltip khi hover
- Hiển thị theo role (Sales không thấy menu Nhân viên)

### 12.4 Header

- **Global Search:** `Ctrl+K` mở modal search toàn hệ thống
- **Notification Bell:** Badge đỏ số lượng chưa đọc, dropdown, click → navigate đến nguồn
- **Avatar menu:** Thông tin cá nhân, Đổi mật khẩu, Đăng xuất

### 12.5 Dashboard

**KPI Cards (4 thẻ ngang):**

| Thẻ | Chỉ số | Màu |
|---|---|---|
| Doanh thu hôm nay | Tổng VNĐ | Xanh |
| Đơn hàng mới | Số đơn | Xanh dương |
| Đang sản xuất | Số lệnh | Cam |
| Tồn kho cảnh báo | Số SP | Đỏ |

**Biểu đồ:**
- Line chart: Doanh thu 7 ngày / 30 ngày
- Bar chart: Sản lượng theo ngày
- Pie chart: Tỷ lệ trạng thái đơn hàng

### 12.6 Bán hàng — Table View

- Filter multi: status, date range, sales person
- Click row → Drawer phải (chi tiết, không reload)
- Drawer: Thông tin đơn, danh sách SP, lịch sử trạng thái, hành động

### 12.7 Sản xuất — Kanban Board

- 4 cột: Chờ xử lý | Đang làm | QC | Hoàn thành
- Card: Tên SP, SL, deadline (đỏ nếu quá hạn), NV phụ trách
- Drag & drop theo luồng cho phép
- Click card → Drawer chi tiết + hành động

### 12.8 Kho

- Bảng tồn kho: Mã SP, Tên, Tồn, Giữ chỗ, Khả dụng, Ngưỡng, Badge trạng thái
- Click row → Modal nhập/xuất/điều chỉnh
- Tab Lịch sử: Timeline giao dịch realtime
- Export Excel
...
| Tính năng | Mô tả |
|---|---|
| SPA | Vue Router, không reload khi chuyển trang |
| Realtime | WebSocket: tồn kho, trạng thái đơn, notification |
| Inline edit | Double-click sửa trực tiếp trường cho phép |
| Keyboard shortcuts | `Ctrl+K` Search, `Ctrl+N` New Order, `Esc` đóng drawer |
| Optimistic UI | Cập nhật UI ngay, rollback nếu API lỗi |
| Skeleton loading | Placeholder khi đang load |
| Empty state | Hình + CTA khi chưa có dữ liệu |

### 12.10 UI theo Role

| Role | Sidebar hiển thị | Dashboard Focus |
|---|---|---|
| Admin | Tất cả | Toàn bộ KPI |
| Sales | Bán hàng, Sản phẩm, Thông báo | Doanh thu cá nhân |
| Production | Sản xuất, Thông báo | Kanban |
| Warehouse | Kho, Sản phẩm, Thông báo | Tồn kho, cảnh báo |
| HR | Nhân viên, Thông báo | KPI nhân viên |

---

## 13. TECH STACK & KIẾN TRÚC TRIỂN KHAI

### 13.1 Tech Stack

| Layer | Technology | Version | Ghi chú |
|---|---|---|---|
| Backend | Laravel | 11.x | Core framework, Events |
| Frontend | Livewire 3 + Alpine | Latest | SSR/SPA-like UI |
| Database | MySQL | 8.0+ | Primary storage |
| Cache/Queue | Redis | 7.x | Cache, Queue, Session |
| Realtime | Laravel Reverb / Pusher | — | WebSocket |
| Auth | Laravel Session Auth | — | Session-based |
| File Storage | Local / AWS S3 | — | Avatar, báo cáo |
| Web Server | Nginx | Latest | Reverse proxy |
| Container | Docker + Docker Compose | — | Dev env |
...
app/Livewire/          # Livewire PHP component classes
├── Dashboard/
├── Sales/
├── Production/
├── Warehouse/
...
### 13.3 Backend Structure (Laravel)

```
app/
├── Http/
│   ├── Controllers/   # API controllers theo module
...
```yaml
services:
  app:       # Laravel + PHP 8.3 (includes Livewire)
  db:        # MySQL 8.0
  redis:     # Redis 7
...
## 14. BẢO MẬT & PHÂN QUYỀN (RBAC)

### 14.1 Authentication

- **Bearer Token** (Laravel Sanctum) trong header `Authorization`
- **Token expiry:** 8 giờ, có thể refresh
- **Password:** Bcrypt hashing, tối thiểu 8 ký tự
- **Rate limiting:** 5 lần sai → khóa 15 phút

### 14.2 Authorization

- Mọi route đều qua middleware `auth` + `can:permission`
- Laravel Policies cho resource-level authorization
- Frontend ẩn/hiện UI theo role (backend vẫn validate)
...
- XSS: Blade & Livewire auto-escape + backend validate
...
- Đăng nhập / đăng xuất
- CRUD đơn hàng, lệnh SX
- Nhập/xuất/điều chỉnh kho
- Thay đổi phân quyền user

---

## 15. HIỆU NĂNG & KHẢ NĂNG MỞ RỘNG

### 15.1 Caching Strategy

| Dữ liệu | Cache | TTL | Invalidate khi |
|---|---|---|---|
| Danh sách sản phẩm | Redis | 10 phút | Tạo/sửa sản phẩm |
| Tồn kho (read) | Redis | 30 giây | Giao dịch kho |
| Dashboard KPI | Redis | 5 phút | Cron job |
| Permissions | Redis | Session | Đổi role |
...
- Queue workers scale độc lập
- Redis → Cluster khi cần
- File storage tách riêng (S3)
- Database read replica khi tải cao
...
- [ ] Module Sản phẩm (CRUD cơ bản)
- [ ] Module Bán hàng (tạo đơn, trạng thái)
- [ ] Module Kho (nhập/xuất/tồn kho)
- [ ] Notification cơ bản
- [ ] Dashboard KPI cơ bản

### Phase 2 — Core Features (Tháng 3-4)
- [ ] Module Sản xuất đầy đủ (Kanban, QC, báo cáo)
- [ ] BOM & Đề xuất NVL
...
### Phase 4 — Extension (Tháng 7+)
- [ ] Mobile app (React Native / Flutter)
- [ ] AI dự đoán tồn kho & doanh thu
- [ ] Integration kế toán
...
*Tài liệu này được duy trì song song với quá trình phát triển. Mọi thay đổi kiến trúc cần được phản ánh trong spec này trước khi implement.*

---
**© ERP Internal System — Confidential**
